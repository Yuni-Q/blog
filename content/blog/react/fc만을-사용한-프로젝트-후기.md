...
## styled-component GTM을 위한 className 노출
...
## 리팩토링 전후 눈에 보이는게 없는데 무엇이 바뀐걸까 ?

- https://wiki.woowa.in/pages/viewpage.action?pageId=214181981

---

### warning

- 개발자 도구에서 Console에 뜨는 수많은 warning들 중 일부를 제거했습니다.
- warning은 치명적인 기능의 장애를 일으키지 않지만 잠재적인 문제를 안고 있기 때문에 가능하다면 제거하는 것이 좋습니다.

---

### url에 한글이 있어도 되는가

- url에 Query 부분에 한글이 들어가 있는데 사용 시에는 문제가 없지만 IE에서 이를 복붙할 경우 문제가 발생합니다.
- 이를 encodeURIComponent해서 url에 사용하고 decodeURIComponent를 사용하여 해결하고 있습니다.
- url에는 영어만 사용할 것을 고민해 보는것도 좋을거 같습니다.(백오피스 개편과 함께 변화될 것으로 기대합니다)

---

### 마우스가 cursor로 바뀌지 않음

- 클릭 가능한 영역에 마우스를 올리면 마우스가 손가락 모양으로 바뀌에 되는데 클릭 영역임에도 손가락으로 바뀌지 않는 경우가 있었습니다.
- 이를 사용자 친화적인 관점에서 손가락으로 표시되게 수정했습니다.
...
- 로그를 페이지 단위로 분리해 뒀었는데 사파리 캐시로 인해 페이지를 새로그리는(새로고침 X) 과정에서 로그가 2번 이상 찍힘
- 로그 부분을 새로그리는 로직보다 상위에서 한번에 관리하는 것으로 수정했습니다.
...
- 가장 대포적인 사례는 실시간 상담 다이얼로그 처럼 다른 곳에서 쓰는 같은 부분들의 코드가 각각 관리되고 있었습니다.
...
- js 코드는 배포 시에 난독화르 하기 때문에 운영에서 제대로 배포되었는지 확인하기 어렵습니다.
- 이를 위해 배포 날짜를 난독화 최상위에 추가함으로써 누구나 쉽게 확인할 수 있도록 수정했습니다.

---

## 이제부터 본격적인 개발 이야기 입니다.
...
- 통일성 없는 코드를 통일화
- 복잡한 코드를 단순화 하기 위한 코드 추가
- 타입스크립트의 장점을 활용하기 위한 타입 정의
- 리드미(프로젝트 내의 설명) 문서 업데이트
- 린트룰(코드 스타일 정리를 위한 규칙) 추가
- 폴더 구조 개선 및 불필요한 파일 제거
...
- 스토리북도 조금씩 준비를 해서 아마도(?) 가능하다면(?) 피트스탑에 이후에는 볼 수 있지 않을까 싶습니다.
