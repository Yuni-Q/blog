...
- hook은 함수형 컴포넌트에서 상태관리나 라이프사이클을 제어하도록 리액트에서 제공하는 함수입니다.
- useState의 경우 함수형 컴포넌트 내에서 컴포넌트의 상태를 변경시킬 수 있습니다
- hook은 함수형 컴포넌트 내부에만 정의 되어야 합니다. 함수형 컴포넌트 밖에서 정의한다면 에러가 발생하게 됩니다.
...
<html lang="en">
  ...
  <body>
    ...
    <script type="text/babel">
      ...
      function isFunctionComponent(element) {
        return typeof element.type === 'function';
      }
      function isProperties(key) {
      ...
      function createDomTree(node) {
        ...
        newNode.children = newNode.children.map(createNode).map(createDomTree);
      ...
      function appendDom(node) {
        ...
        children.map(appendDom).forEach((child) => dom.appendChild(child));
...
- [자바스크립트로 만들어 보는 리액트 프레임워크 - 3. hook](https://bgpark.tistory.com/168)
