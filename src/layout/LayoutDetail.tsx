...
body {
    ...
    theme === THEME.LIGHT ? lightBackgroundColor : darkBackgroundColor};
	text-shadow: ${({ theme }) =>
    theme === THEME.LIGHT ? lightTextShadow : darkTextShadow};
	color: ${({ theme }) =>
    theme === THEME.LIGHT ? 'inherit' : darkLightFontColor};
	...
	.bio {
		...
		.author-introduction {
			color: ${({ theme }) =>
        theme === THEME.LIGHT ? '#7d7d7d' : darkMiddleFontColor};
		}
		a {
	...
	.category-container {
      ...
      theme === THEME.LIGHT
        ...
        theme === THEME.LIGHT
          ...
          : darkCategoryBorderColor};
			background-color: ${({ theme }) =>
        theme === THEME.LIGHT ? '#fff' : darkCategoryItemColor};
			-webkit-box-shadow: 0px 1px 1px rgba(255, 255, 255, 0.1);
			box-shadow: 0px 1px 1px rgba(255, 255, 255, 0.1);
			a {
				color: ${({ theme }) =>
          theme === THEME.LIGHT ? '#666' : darkCategoryTextColor};
			}
			&[aria-selected='true'] {
				border: 2px solid ${({ theme }) =>
          theme === THEME.LIGHT ? '#909da1' : darkCategoryHighlightBorderColor};
				font-weight: bolder;
			}
			&[aria-selected='true'] a {
				color: ${({ theme }) =>
          theme === THEME.LIGHT ? '#636c6e' : darkCategoryHighlightTextColor};
			}
		}
	}
	...
	.footer {
		...
		a {
			color: ${({ theme }) =>
        theme === THEME.LIGHT ? '#333' : darkLightestFontColor};
		}
	}

	blockquote {
		border-left: ${({ theme }) =>
      theme === THEME.LIGHT ? 'none' : '4px solid rgba(255, 255, 255, 0.822)'};
	}

	h1,
	h2 {
		border-bottom-color: ${({ theme }) =>
      theme === THEME.LIGHT ? '#none' : 'rgba(255, 255, 255, 0.3)'};
	}

	.navigator {
		a {
			background-color: ${({ theme }) =>
        theme === THEME.LIGHT ? '#fceft7' : navigatorBackgroundColor};
			color: ${({ theme }) =>
        theme === THEME.LIGHT ? '#cc007a' : navigatorTextColor};
		}
	}
}
`;

export const LayoutDetail = ({ title, children }) => {
  const rootPath = '/';
